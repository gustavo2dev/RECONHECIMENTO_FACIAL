...
start "Reconhecimento facial" /min /D "%~dp0backend" "%~dp0.venv\Scripts\pythonw.exe" server.py
start "Painel administrativo" /min /D "%~dp0backend\admin" "%~dp0.venv\Scripts\pythonw.exe" admin_server.py
timeout /t 2 /nobreak >nul
start "Painel administrativo no navegador" http://localhost:5000
start "Aplicacao Electron" /D "%~dp0frontend" cmd /c npm start
